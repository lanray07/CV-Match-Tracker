# Privacy Policy

CV Match Tracker stores application records, CV files, cover letters, recruiter details, reminders, notes, and job descriptions locally on your device.

Version 1.0 does not require an account, does not use analytics, does not track users, and does not send your data to a server.

Local notifications may be used for reminders that you create in the app. Uploaded documents remain in the app's local storage on your device.

Future versions may add optional iCloud sync. This privacy policy will be updated before any data collection or sync feature is released.
